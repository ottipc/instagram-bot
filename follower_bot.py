...
username = "sophia_bianchil@protonmail.com"
password = "NpCqX595RYft8rO5fnr6"
...
def bot():
    ...
    session = InstaPy(username=username, password=password, want_check_browser=False)
    ...
    with smart_run(session):
        session.like_by_tags(['fashion, model'],
                             amount=50)  # Specifying tags to search posts for, to ultimately like them.
        session.set_do_follow(False,
                              percentage=r.randint(20, 70))  # Setting percent chance that the bot will follow the user.
        session.set_do_comment(True, percentage=r.randint(10,
                                                          30))  # Setting percent chance that the bot will leave a comment on the post.
        session.set_comments(["Great!", "Love this", "Nice", "I like this",
                              "Liked!"])  # Specifying what the actual comments should be if the bot decides to leave them
